...
function switchPriceAddons() {
  const yearlyPriceAddons = [10, 20, 30];
  const monthlyPriceAddons = [1, 2, 3];
  const Addonsprice = document.querySelectorAll(".price");

  const billingType = localStorage.getItem("billingType");

  if (billingType === "yearly") {
    Addonsprice[0].innerHTML = `$${yearlyPriceAddons[0]}/yr`;
    Addonsprice[1].innerHTML = `$${yearlyPriceAddons[1]}/yr`;
    Addonsprice[2].innerHTML = `$${yearlyPriceAddons[2]}/yr`;
    setTime(true);
  } else {
    Addonsprice[0].innerHTML = `$${monthlyPriceAddons[0]}/mo`;
    Addonsprice[1].innerHTML = `$${monthlyPriceAddons[1]}/mo`;
    Addonsprice[2].innerHTML = `$${monthlyPriceAddons[2]}/mo`;
    setTime(false);
  }
}
